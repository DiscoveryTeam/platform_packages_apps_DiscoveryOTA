...
package org.discovery.ota.receivers;
...
import org.discovery.ota.DiscoveryOTA;
import org.discovery.ota.R;
import org.discovery.ota.RomUpdate;
import org.discovery.ota.activities.AddonActivity;
import org.discovery.ota.activities.AvailableActivity;
import org.discovery.ota.tasks.LoadUpdateManifest;
import org.discovery.ota.utils.Constants;
import org.discovery.ota.utils.Preferences;
import org.discovery.ota.utils.Utils;
...
public class AppReceiver extends BroadcastReceiver implements Constants{
	...
	public void onReceive(Context context, Intent intent) {
		...
		if (action.equals(DownloadManager.ACTION_DOWNLOAD_COMPLETE)) {
			...
			Set<Integer> set = DiscoveryOTA.getAddonDownloadKeySet();
			Iterator<Integer> iterator = set.iterator();

			while (iterator.hasNext() && isAddonDownload != true) {
				int nextValue = iterator.next();
				if (id == DiscoveryOTA.getAddonDownload(nextValue)) {
			...
			if (isAddonDownload) {
				...
				if (DownloadManager.STATUS_SUCCESSFUL != cursor.getInt(statusIndex)) {
					...
					DiscoveryOTA.removeAddonDownload(keyForAddonDownload);
